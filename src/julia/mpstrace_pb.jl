...
struct Tracefile
    ...
    inst_to_conv::Vector{Pair}
    state_evolution::Vector{mpssim_pb.MPSState}
end
PB.default_values(::Type{Tracefile}) = (;mimiq_circuit = nothing, converted_circuit = nothing, inst_to_conv = Vector{Pair}(), state_evolution = Vector{mpssim_pb.MPSState}())
PB.field_numbers(::Type{Tracefile}) = (;mimiq_circuit = 1, converted_circuit = 2, inst_to_conv = 3, state_evolution = 4)
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:Tracefile})
    ...
    inst_to_conv = PB.BufferedVector{Pair}()
    ...
    return Tracefile(mimiq_circuit[], converted_circuit[], inst_to_conv[], state_evolution[])
end
...
function PB.encode(e::PB.AbstractProtoEncoder, x::Tracefile)
    ...
    !isempty(x.inst_to_conv) && PB.encode(e, 3, x.inst_to_conv)
...
function PB._encoded_size(x::Tracefile)
    ...
    !isempty(x.inst_to_conv) && (encoded_size += PB._encoded_size(x.inst_to_conv, 3))
